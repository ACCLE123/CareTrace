...
async function boot() {
  const identities = await fetch('/api/demo-identities').then(r => r.json());
...
$('#identity').onchange = async (e) => { const identities = await fetch('/api/demo-identities').then(r => r.json()); state.user = identities.find(i => i.id === e.target.value); $('#new-note').style.display = canAdd() ? '' : 'none'; $('#note-form').classList.add('hidden'); try { await refresh(); toast(`Viewing as ${state.user.role}. Server permissions applied.`); } catch(err) { toast(err.message); } };
